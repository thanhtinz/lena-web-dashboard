...
import { boostConfigs } from '@/lib/schema';
...
export async function GET(
  ...
  try {
    ...
    const config = await db.select()
      .from(boostConfigs)
      .where(eq(boostConfigs.serverId, id))
      .limit(1);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch boost configuration' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (existing.length === 0) {
      const result = await db.insert(boostConfigs).values({
        serverId: id,
        channelId,
        message,
        embedName,
      ...
      const result = await db.update(boostConfigs)
        .set({
          channelId,
          message,
          embedName,
    ...
    return NextResponse.json(
      { error: 'Failed to update boost configuration' },
      { status: 500 }
    );
  }
}
