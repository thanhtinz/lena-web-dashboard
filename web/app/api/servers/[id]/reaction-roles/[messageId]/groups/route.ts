...
import { reactionRoleMessages, reactionRoleGroups } from '@/lib/schema';
...
export async function GET(
  ...
  const groups = await db.select().from(reactionRoleGroups)
    .where(eq(reactionRoleGroups.messageConfigId, message[0].id));
...
export async function POST(
  ...
  try {
    ...
    const { groupName, groupType, maxRoles, emojiIds } = body;

    if (!groupName || !groupType || !emojiIds || emojiIds.length === 0) {
    ...
    await db.insert(reactionRoleGroups).values({
      ...
      groupName,
      groupType,
      maxRoles: groupType === 'limit' ? maxRoles : 1,
      emojiIds
    });
