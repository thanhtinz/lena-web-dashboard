...
import { reactionRoleMessages, reactionRoleBlacklist } from '@/lib/schema';
...
export async function GET(
  ...
  const blacklist = await db.select().from(reactionRoleBlacklist)
    .where(eq(reactionRoleBlacklist.messageConfigId, message[0].id));
...
export async function POST(
  ...
  try {
    ...
    await db.insert(reactionRoleBlacklist).values({
